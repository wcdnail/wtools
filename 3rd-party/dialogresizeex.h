...
public: 
    ...
    CDialogResizeEx(void)
        : m_size(0, 0)
        , m_hKeyParent(nullptr)
        , m_lpszKeyName(nullptr)
    {
    }
    ...
    void DlgResize_InitEx(bool bAddGripper = true, bool bUseMinTrackSize = true, DWORD dwForceStyle = WS_CLIPCHILDREN)
    {
        ...
        pT->DlgResize_Init(bAddGripper, bUseMinTrackSize, dwForceStyle);
        ...
        if (m_size.cx != 0 && m_size.cy != 0)
        {
            ...
            pT->DlgResize_UpdateLayout(rectClient.Width(), rectClient.Height());
        }
    }
    ...
    ATL::CString FormatWidthValueName(void) const
    {
        const T* pT = static_cast<const T*>(this);
        ATL::CString strValueName;
        strValueName.Format(_T("dialog_%d_cx"), pT->IDD);
        return strValueName;
    }

    ATL::CString FormatHeightValueName(void) const
    {
        const T* pT = static_cast<const T*>(this);
        ATL::CString strValueName;
    ...
    LRESULT OnDestroy(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
    {
        ...
        // Save the window size, ONLY if NOT maximized
        if(TRUE != pT->IsZoomed()) {
            CRect rect;
            ...
            if (m_hKeyParent != nullptr && m_lpszKeyName != nullptr) {
