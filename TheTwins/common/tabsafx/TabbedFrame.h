...
// - Update CTabbedFrameImpl::OnRemoveLastTab to call this->this->Invalidate()
//
...
static constexpr int nForceTabAreaHeight = 32;
...
// Member variables
protected:
    using ItemType = TTabCtrl::ItemType;
...
public:
    ...
    void KeepTabsHidden(bool bKeepTabsHidden = true)
    {
        if(this->m_bKeepTabsHidden != bKeepTabsHidden)
        {
            ...
            // CalcTabAreaHeight will end up doing UpdateLayout and this->Invalidate
            T* pT = static_cast<T*>(this);
    ...
    void SetTabAreaHeight(int nNewTabAreaHeight)
    {
        ...
        {
            this->m_nTabAreaHeight = nForceTabAreaHeight;
...
public:
    ...
    int AddBitmap(_U_STRINGorID bitmap, HBITMAP hBitmapMask, HMODULE hModule/*  = _Module.GetResourceInstance() */)
    {
        HBITMAP hBitmap = (HBITMAP)::LoadImage(
            hModule,
            bitmap.m_lpstr,
    ...
    int AddTabWithBitmap(HWND hWnd, LPCTSTR sTabText, _U_STRINGorID bitmap, HBITMAP hBitmapMask, HMODULE hModule/*  = _Module.GetResourceInstance() */)
    {
        if(hWnd == NULL)
        {
            return -1;
    ...
    int AddTab(HWND hWnd, LPCTSTR sTabText, int nImageIndex = -1)
    {
        ...
        ItemType* pItem = this->m_TabCtrl.CreateNewItem();
        if(pItem) {
            ...
            nNewTabIndex = this->m_TabCtrl.InsertItem((int)nOldCount, pItem);
...
// need conditional code because types don't match in winuser.h
    ...
    BOOL UpdateTabText(HWND hWnd, LPCTSTR sText = NULL)
    {
        ...
        if(nTab >= 0)
        {
            ItemType* pItem = this->m_TabCtrl.GetItem(nTab);
...
protected:
    ...
    using ItemType = customTabOwnerClass::ItemType;
...
public:
    ...
    LRESULT OnClick(int /*idCtrl*/, LPNMHDR pnmh, BOOL& bHandled)
    {
        ...
        if(pnmh && (this->m_TabCtrl == pnmh->hwndFrom))
        {
            ...
            {
                ItemType* pItem = this->m_TabCtrl.GetItem(item->iItem);
    ...
    LRESULT OnAcceptItemDrag(int /*idCtrl*/, LPNMHDR pnmh, BOOL& bHandled)
    {
        ...
        if(pnmh && (this->m_TabCtrl == pnmh->hwndFrom))
        {
            ...
            {
                ItemType* pItem = this->m_TabCtrl.GetItem(item->iItem2);
                if(pItem->UsingTabView())
                {
                    ::SetFocus(pItem->GetTabView());
                }
    ...
    LRESULT OnSelChange(int /*idCtrl*/, LPNMHDR pnmh, BOOL& bHandled)
    {
        ...
        if(pnmh && (this->m_TabCtrl == pnmh->hwndFrom))
        {
            ...
            if(nNewTab >= 0)
            {
                ItemType* pItem = this->m_TabCtrl.GetItem(nNewTab);
                if(pItem->UsingTabView())
                {
...
public:
    ...
    {
        if(this->PreTranslateMessage(pMsg))
            return TRUE;
