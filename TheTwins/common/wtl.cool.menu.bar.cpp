...
CCoolMenuBar::CCoolMenuBar()
    : m_Font(::CreateFont(SIDEBAR_WIDTH - 2, 0, 900, 900, 0, FALSE, FALSE, FALSE, 0, OUT_DEFAULT_PRECIS, CLIP_MASK, PROOF_QUALITY, FF_DONTCARE, _T(SIDEBAR_FONT)))
{}
...
void CCoolMenuBar::DrawMenuText(CDCHandle& dc, RECT& rc, LPCTSTR lpstrText, COLORREF colorText, COLORREF colorAccellerator)
{
    int nTab = -1;
...
// item structure.
void CCoolMenuBar::DrawSideBar(CDCHandle& dc, RECT& grrect, LPCTSTR sidebarText)
{
    RECT rct;
    dc.GetClipBox(&rct);
    ...
    if (sidebarText)
    {
        ...
        HFONT fontold = dc.SelectFont(m_Font);
        ...
        dc.SetTextColor( 0x0 );
        dc.DrawText(sidebarText, (int)_tcslen(sidebarText), &dims, DT_SINGLELINE | DT_BOTTOM);
        dims.top -= 1;
        dims.left -= 1;
        dims.right -= 1;
        dims.bottom -= 1;
...
void CCoolMenuBar::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
    ...
    else		// not a separator
    {
        ...
        if(lpDrawItemStruct->itemID == SidebarId)
        {
        ...
        if(bDisabled && (!bSelected || colorText == colorBG))
        {
            ...
            Super::DrawMenuText(dc, rcDisabled, pmd->lpstrText, ::GetSysColor(COLOR_3DHILIGHT));
        }
...
void CCoolMenuBar::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
    ...
    {
        // == Begin, Ramon Smits 22-04-2000
        if(lpMeasureItemStruct->itemID == SidebarId)
        {
