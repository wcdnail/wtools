...
WCDAFX_API int PPMessageBox(HWND hwnd,
				            LPCTSTR lpszMessage,
...
WCDAFX_API int PPMessageBoxIndirect(const PPMSGBOXPARAMS * pMsgBox)
{
	_ASSERT(pMsgBox);
...
int CPPMessageBoxTemplate::Display()
{
	...
	int nResult = (int)::DialogBoxIndirectParam(hInstance, pdlg, m_MsgParam.hParentWnd, (DLGPROC)MsgBoxDlgProc, (LPARAM)this);
...
{
	CPPMessageBoxTemplate * pDlg = (CPPMessageBoxTemplate*)::GetWindowLongPtr(hwnd, GWLP_USERDATA);
...
void CPPMessageBoxTemplate::OnInitDialog(HWND hwnd)
{
	...
	int nWidth = (int)m_vecButtons.size() * (m_dwSizes [PPMSGBOX_SIZE_BTNX] + m_dwSizes [PPMSGBOX_SIZE_CTRLX]) - m_dwSizes [PPMSGBOX_SIZE_CTRLX];
...
SIZE CPPMessageBoxTemplate::PrepareMessageBox(HWND hwnd)
{
	...
	if ((m_MsgParam.pControlBk.nEffectBk >= 0) || (m_MsgParam.pControlBk.nSepType != PPMSGBOX_SEP_NONE))
	{
		...
		//ENG: Control separator will be drawn
...
//				SetButtonText(dwButton, m_MsgParam.sCustomButtons.Mid(i, nIndex - i));
	...
	szTemp.cx = m_dwSizes [PPMSGBOX_SIZE_CTRLX] + (int)m_vecButtons.size() * (m_dwSizes [PPMSGBOX_SIZE_CTRLX] + m_dwSizes [PPMSGBOX_SIZE_BTNX]);
	sz.cx = max(sz.cx, szTemp.cx);
