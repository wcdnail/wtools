...
//      to provide maximum CString-compatibility, this code needs them as well.
...
inline SS_NOTHROW int sslen(const CharString& s)
{
	return static_cast<int>(s.length());
}
inline SS_NOTHROW int sslen(const WidecharString& s)
{
	return static_cast<int>(s.length());
}
...
typedef WidecharString::size_type		SW_SIZETYPE;
typedef WidecharString::pointer		SW_PTRTYPE;  
...
inline void	ssasn(std::string& sDst, const WidecharString& sSrc)
{
	if ( sSrc.empty() )
	{
		sDst.erase();
...
inline void	ssasn(WidecharString& sDst, const WidecharString& sSrc)
{
	if ( sDst.c_str() != sSrc.c_str() )
	{
		sDst.erase();
...
inline void	ssasn(WidecharString& sDst, PCWSTR pW)
{
	// Watch out for NULLs, as always.
...
#undef StrSizeType
inline void	ssasn(WidecharString& sDst, const std::string& sSrc)
{
	if ( sSrc.empty() )
	{
		sDst.erase();
...
inline void	ssasn(WidecharString& sDst, PCSTR pA)
{
	int nSrc	= sslen(pA);
...
inline void ssasn(WidecharString& sDst, const int nNull)
{
	UNUSED(nNull);
	ASSERT(nNull==0);
	sDst.assign(L"");
...
// -----------------------------------------------------------------------------
inline void	ssadd(std::string& sDst, const WidecharString& sSrc)
{
	int nSrc	= static_cast<int>(sSrc.size());
...
inline void	ssadd(WidecharString& sDst, const WidecharString& sSrc)
{
	sDst += sSrc;
}
inline void	ssadd(WidecharString& sDst, const std::string& sSrc)
{
	if ( !sSrc.empty() )
	{
		int nSrc	= static_cast<int>(sSrc.size());
...
inline void	ssadd(WidecharString& sDst, PCSTR pA)
{
	int nSrc		= sslen(pA);
...
inline void	ssadd(WidecharString& sDst, PCWSTR pW)
{
	if ( pW )
	{
		// If the string being added is our internal string or a part of our
...
//    PCWSTR FmtArg(const WidecharString& arg) { return arg.c_str(); }
...
public:
	...
	CStdStr(const WidecharString& str)
	{
		ssasn(*this, SSREF(str));
	}
	...
	MYTYPE& operator=(const WidecharString& str)
	{
		ssasn(*this, str);
		return *this;
	}
...
//		}
...
#ifndef SS_ANSI
	void FormatMessage(PCMYSTR szFormat, ...) noexcept(false)
	{
		va_list argList;
	...
	void FormatMessage(UINT nFormatId, ...) noexcept(false)
	{
		MYTYPE sFormat;
	...
	{
		return Mid(nFirst, (int)this->size() - (int)static_cast<MYSIZE>(nFirst));
	}
...
typedef CStdStr<wchar_t>	CStdStringW;	// a better WidecharString
...
template<>
struct FmtArg<WidecharString>
{
    explicit FmtArg(const WidecharString& arg) : a_(arg) {}
    PCWSTR operator()() const { return a_.c_str(); }
    const WidecharString& a_;
private:
    FmtArg<WidecharString>& operator=(const FmtArg<WidecharString>&) {return *this;}
};
