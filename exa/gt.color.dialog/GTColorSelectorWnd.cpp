...
GTColorSelectorWnd::GTColorSelectorWnd()
{
	...
	bmp_handle = nullptr;
	m_bTracking = false;
	picker_cursor = LoadCursor (nullptr, IDC_CROSS);
...
void GTColorSelectorWnd::OnSize(UINT nType, int cx, int cy) 
{
	...
	bmp_handle = CreateDIBSection (
		nullptr, &bmp_info, DIB_RGB_COLORS, (void **) &bmp_data, nullptr, 0);
	if (bmp_handle == nullptr)
	{
		return;
	}
...
LRESULT GTColorSelectorWnd::OnSelectBulletOK(WPARAM wParam, LPARAM lParam)
{
	UNREFERENCED_PARAMETER(wParam);
	UNREFERENCED_PARAMETER(lParam);
	auto const* slider = static_cast<GTColorSliderWnd*>(pNotifyWnd);
	...
	case	modes_hsv_hue:
		{
			...
			m_Panel.RGB2HSV(
				...
				&hue, &sat, &val);
			x = (int) ceil(sat * (double)scale_hsv_sat);
			y = (int) ceil(val * (double)scale_hsv_value);
			comp = (int) ceil(hue * (double)scale_hsv_hue);
		}
		break;
	...
	case	modes_hsv_sat:
		{
			...
			m_Panel.RGB2HSV (
				...
				blue / (double) max_rgb_blue,
				&hue, &sat, &val);
			x = (int) ceil (hue * (double)scale_hsv_hue);
			y = (int) ceil (val * (double)scale_hsv_value);
			comp = (int) ceil (sat * (double)scale_hsv_sat);
		}
		break;
	...
	case	modes_hsv_value:
		{
			...
			m_Panel.RGB2HSV (
				...
				&hue, &sat, &val);
			x = (int) ceil (hue * (double)scale_hsv_hue);
			y = (int) ceil (sat * (double)scale_hsv_sat);
			comp = (int) ceil (val * (double)scale_hsv_value);
		}
		break;
