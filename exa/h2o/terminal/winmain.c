...
typedef HRESULT (WINAPI *PDWMEXTENDFRAMEINTOCLIENTAREA)(HWND, const MARGINS *);

static HRESULT WINAPI DmyDwmIsCompositionEnabled(BOOL *) { return E_NOTIMPL; }
static HRESULT WINAPI DmyDwmExtendFrameIntoClientArea(HWND, const MARGINS *) { return E_NOTIMPL; }

static PDWMISCOMPOSITIONENABLED pDwmIsCompositionEnabled = DmyDwmIsCompositionEnabled;
static PDWMEXTENDFRAMEINTOCLIENTAREA pDwmExtendFrameIntoClientArea = DmyDwmExtendFrameIntoClientArea;
...
load_funcs(void)
{
  ...
  if (len && len < MAX_PATH - sizeof("\\dwmapi.dll")) {
    ...
    pDwmIsCompositionEnabled =
      (PDWMISCOMPOSITIONENABLED)GetProcAddress(dwm, "DwmIsCompositionEnabled");
    pDwmExtendFrameIntoClientArea =
      (PDWMEXTENDFRAMEINTOCLIENTAREA)GetProcAddress(dwm, "DwmExtendFrameIntoClientArea");
  }
}
...
win_set_title(char *title)
{
  ...
  CStringW wtitle;
  if (cs_mbstowcs(wtitle.GetBufferSetLength(len), title, len) >= 0)
...
win_copy_title(void)
{
  ...
  CStringW title;
  len = GetWindowTextW(wnd, title.GetBufferSetLength(len + 1), len + 1);
  title.ReleaseBufferSetLength(len + 1);
...
win_save_title(void)
{
  int len = GetWindowTextLengthW(wnd);
  CStringW title;
  int len2 = GetWindowTextW(wnd, title.GetBufferSetLength(len + 1), len + 1);
  title.ReleaseBufferSetLength(len2);
...
win_restore_title(void)
{
  ...
  CStringW& title = titles[--titles_i];
...
wnd_enum_proc(HWND curr_wnd, LPARAM unused(lp)) {
  if (curr_wnd != wnd && !IsIconic(curr_wnd)) {
    ...
    if (class_atom == curr_wnd_info.atomWindowType) {
      first_wnd = first_wnd ? first_wnd : curr_wnd;
      last_wnd = curr_wnd;
    }
  }
  return true;
...
flash_taskbar(bool enable)
{
  ...
  if (enable != enabled) {
    FLASHWINFO fi = {
          sizeof(FLASHWINFO),
          wnd,
          enable ? FLASHW_TRAY | FLASHW_TIMER : FLASHW_STOP,
          1,
          0
      };
    FlashWindowEx(&fi);
...
update_glass(void)
{
  if (pDwmExtendFrameIntoClientArea) {
    ...
    MARGINS mns = { enabled ? -1 : 0, 0, 0, 0 };
    pDwmExtendFrameIntoClientArea(wnd, &mns);
  }
}
  ...
  switch (message) {
    ...
    when WM_IME_COMPOSITION:
      if (lp & GCS_RESULTSTR) {
        ...
        if (len > 0) {
          CStringW buf;
          ImmGetCompositionStringW(imc, GCS_RESULTSTR, buf.GetBufferSetLength(len), len);
          child_sendw(buf, len);
        }
        return 1;
      }
...
show_msg(FILE *stream, string msg)
{
  if (fputs(msg, stream) < 0 || fflush(stream) < 0)
    MessageBoxA(0, msg, APPNAME, MB_OK);
}

static void // no_return __attribute__((format(printf, 1, 2)))
error(char *format, ...) 
{
  char *msg;
  va_list va;
...
main(int argc, char *argv[])
{
  ...
  home = getenv("HOME"); // TODO: get HOME from boost::filesystem
...
//#endif
  ...
  */
  ...
  // The window class.
  WNDCLASSEXW class_atom = {};
    class_atom.cbSize = sizeof(WNDCLASSEXW);
    class_atom.style = 0;
    class_atom.lpfnWndProc = win_proc;
    class_atom.cbClsExtra = 0;
    class_atom.cbWndExtra = 0;
    class_atom.hInstance = inst;
    class_atom.hIcon = large_icon ? NULL : LoadIcon(inst, MAKEINTRESOURCE(IDI_MAINICON));
    class_atom.hIconSm = small_icon;
    class_atom.hCursor = LoadCursor(null, IDC_IBEAM);
    class_atom.hbrBackground = null;
    class_atom.lpszMenuName = null;
    class_atom.lpszClassName = wclass;
  
  RegisterClassExW(&class_atom);
