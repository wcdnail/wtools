...
{  
  ...
  return fq[(int)cfg.font_smoothing];
    //(uchar[]){
  ...
  */
  {
    ...
    und_oldbm = (HBITMAP)SelectObject(und_dc, (HGDIOBJ)und_bm);
...
win_paint(void)
{
  ...
  if (p.fErase || p.rcPaint.left < PADDING ||
    ...
    HBRUSH oldbrush = (HBRUSH)SelectObject(dc, (HGDIOBJ)CreateSolidBrush(bg_colour));
    HPEN oldpen = (HPEN)SelectObject(dc, (HGDIOBJ)CreatePen(PS_SOLID, 0, bg_colour));
...
do_update(void)
{
  ...
  if (cfg.scrollbar && term.show_scrollbar) {
    ...
    SCROLLINFO si = {0};
      ...
      si.nMin = 0;
      si.nMax = lines + term.rows - 1;
      si.nPage = term.rows;
      si.nPos = lines + term.disptop;
...
win_text(int x, int y, wchar *text, int len, uint attr, int lattr)
{
 ...
 /* Only want the left half of double width lines */
  ...
  uint fgi = (attr & ATTR_FGMASK) >> ATTR_FGSHIFT;
  uint bgi = (attr & ATTR_BGMASK) >> ATTR_BGSHIFT;
  ...
  bool has_cursor = (0 != (attr & (TATTR_ACTCURS | TATTR_PASCURS)));
 ...
 /* Check whether the text has any right-to-left characters */
  ...
  if (has_rtl) {
    ...
    */
    ...
    boost::scoped_array<char> classes(new char[len]);
    memset(classes.get(), GCPCLASS_NEUTRAL, len);
    
    GCP_RESULTSW gcpr = {0};
      gcpr.lStructSize = sizeof(GCP_RESULTSW);
      gcpr.lpClass = classes.get();
      gcpr.lpGlyphs = text;
      gcpr.nGlyphs = len;
  ...
  bool combining = (0 != (attr & TATTR_COMBINING));
  int width = char_width * (combining ? 1 : len);
  RECT box = {0};
    box.left = x;
    box.top = y;
    box.right = min(x + width, font_width * term.cols + PADDING);
    box.bottom = y + font_height;
  
 /* Array with offsets between neighbouring characters */
  boost::scoped_array<int> dxs(new int[len]);
 ...
 /* Finally, draw the text */
  SetBkMode(dc, OPAQUE);
  ExtTextOutW(dc, x, yt, eto_options | ETO_OPAQUE, &box, text, len, dxs.get());

 /* Shadow bold */
  if (bold_mode == BOLD_SHADOW && (attr & ATTR_BOLD)) {
    SetBkMode(dc, TRANSPARENT);
    ExtTextOutW(dc, x + 1, yt, eto_options, &box, text, len, dxs.get());
  }
 ...
 /* Manual underline */
  if (lattr != LATTR_TOP &&
    ...
    HPEN oldpen = (HPEN)SelectObject(dc, (HGDIOBJ)CreatePen(PS_SOLID, 0, fg));
    MoveToEx(dc, x, y + dec, null);
    LineTo(dc, x + len * char_width, y + dec);
    oldpen = (HPEN)SelectObject(dc, (HGDIOBJ)oldpen);
    DeleteObject(oldpen);
  }
  
  if (has_cursor) {
    HPEN oldpen = (HPEN)SelectObject(dc, (HGDIOBJ)CreatePen(PS_SOLID, 0, cursor_colour));
    switch(term_cursor_type()) {
      ...
      when CUR_LINE: {
        ...
        if (attr & TATTR_ACTCURS) {
          HBRUSH oldbrush = (HBRUSH)SelectObject(dc, (HGDIOBJ)CreateSolidBrush(cursor_colour));
          ...
          {
            POINT pt[2] = {{x, y + dy}, {x + caret_width, y + dy}};
            Polyline(dc, pt, 2);
          }
        }
      }
...
win_check_glyphs(wchar *wcs, uint num)
{
  ...
  boost::scoped_array<ushort> glyphs(new ushort[num]);
  GetGlyphIndicesW(dc, wcs, num, glyphs.get(), true);
...
win_combine_chars(wchar c, wchar cc)
{
  wchar cs[2];
  wchar chars[2] = {c, cc};
  int len = FoldStringW(MAP_PRECOMPOSED, chars, 2, cs, 2);
  return len == 1 ? *cs : 0;
}
