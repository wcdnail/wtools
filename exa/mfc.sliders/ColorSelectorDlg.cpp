...
BOOL CColorSelectorDlg::OnInitDialog()
{
    ...
    m_wndSpectrum.Create(NULL, _T("Spectrum"), WS_CHILD|WS_VISIBLE, rc, this, 0x1245);
...
void CColorSelectorDlg::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
    ...
    CString str;
    str.Format(_T(" Similar to %s "), m_wndSpectrum.GetNameOfClosestColor(cr));
...
void CColorSelectorDlg::OnBtnCopyValueToClipboard() 
{
    ...
    CString strValue;
    const CString name = m_wndSpectrum.GetNameOfClosestColor(cr, &crNamed);
    strValue.Format(
        _T("RGB(%d, %d, %d);\t// similar to %s (whose exact value is RGB(%d, %d, %d)); ")
        _T("COLORREF value is 0x%08X; web value is #%02X%02X%02X"),
        r, g, b, name,
        GetRValue(crNamed), GetGValue(crNamed), GetBValue(crNamed),
        cr, r, g, b);
    ::EmptyClipboard();
    const HGLOBAL hClipboardData = ::GlobalAlloc(GMEM_DDESHARE, strValue.GetLength() + 1);
    const PTSTR pchData = static_cast<PTSTR>(::GlobalLock(hClipboardData));
    _tcscpy_s(pchData, strValue.GetLength(), strValue.GetString());
