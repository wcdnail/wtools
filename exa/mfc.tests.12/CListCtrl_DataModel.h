...
public:
	...
	void InitDataModel()
	{
		...
		{
			std::vector<CListCtrl_DataRecord> rowset(m_Records);
	...
	std::vector<CString> GetCountries() const
	{
		std::vector<CString> countries;
		for(size_t rowId = 0 ; rowId < m_Records.size(); ++rowId)
	...
	std::vector<CListCtrl_DataRecord>& GetRecords() { return m_Records; }
