...
BOOL CRedirector::LaunchChild(LPCTSTR pszCmdLine, HANDLE hStdOut, HANDLE hStdIn, HANDLE hStdErr)
{
	PROCESS_INFORMATION pi;
	STARTUPINFO si;

	ATLASSERT(pszCmdLine != NULL);
...
void CRedirector::WriteStdOut(PCSTR pszOutput)
{
	ATLTRACE("%s", pszOutput);
}

void CRedirector::WriteStdError(PCSTR pszError)
{
	ATLTRACE("%s", pszError);
}
