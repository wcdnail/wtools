...
{
    if constexpr (false) { // dlg has no wnd frame
        CRect rect;
        GetClientRect(&rect);
        ClientToScreen(rect);
...
void CMainDlg::DoDrawRGB()
{
    ...
    // left side components, extended with g_IntMult bits
    int     left_red, left_green, left_blue;
    // left side component advance, extended with g_IntMult bits
    ...
    // current components, extended with g_IntMult bits
    int     red, green, blue;
    // advance, extended with g_IntMult bits
...
void SColour::RGB_F_HTML(CString szValue)
{
    ...
    const int size = static_cast<int>(hex.size());
    for (int i = 0; i < 3; i++) {
