...
public:
   ...
   LRESULT OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
   {
      ...
      m_ctrlToolBar.InsertButton(0, IDC_SEARCHDROP, BTNS_BUTTON | BTNS_DROPDOWN, TBSTATE_ENABLED, 0,
          reinterpret_cast<INT_PTR>(nullptr),
...
//////////////////////////////////////////////////////////////////////
// CImageSizeSliderCtrl
//

typedef CWinTraits<WS_POPUPWINDOW | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, WS_EX_TOOLWINDOW> CImageSizeSliderWinTraits;

class CImageSizeSliderCtrl : 
   public ATL::CWindowImpl<CImageSizeSliderCtrl, ATL::CWindow, CImageSizeSliderWinTraits>
{
public:
   DECLARE_WND_CLASS(_T("WTL_ImageSideSlider"))

   enum { UPDATE_TIMERID = 43 };

   ATL::CContainedWindowT<CTrackBarCtrl> m_ctrlSlider;
   int m_iLastPos;

   CImageSizeSliderCtrl() : m_ctrlSlider(this, 1), m_iLastPos(0)
   {
   }

   BEGIN_MSG_MAP(CImageSizeSliderCtrl)
      MESSAGE_HANDLER(WM_CREATE, OnCreate)
      MESSAGE_HANDLER(WM_DESTROY, OnDestroy)
      MESSAGE_HANDLER(WM_SIZE, OnSize)
      MESSAGE_HANDLER(WM_TIMER, OnTimer)
   ALT_MSG_MAP( 1 )
      MESSAGE_HANDLER(WM_KILLFOCUS, OnSliderKillFocus)
      MESSAGE_HANDLER(WM_LBUTTONUP, OnSliderKillFocus)
   END_MSG_MAP()
   ...
   LRESULT OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
   {
      ...
      m_iLastPos = (MAX_ZOOM - MIN_ZOOM) / 2;
      m_ctrlSlider.Create(m_hWnd, rcDefault, NULL, WS_VISIBLE | WS_CHILD | TBS_VERT | TBS_NOTICKS);
      m_ctrlSlider.SetRange(MIN_ZOOM, MAX_ZOOM);
      m_ctrlSlider.SetPos(m_iLastPos);
      m_ctrlSlider.SetFocus();
      // Simulate a button-click so trackbar goes immediately into
      // tracking mode...
      POINT pt = { 0 };
      ::GetCursorPos(&pt);
      m_ctrlSlider.ScreenToClient(&pt);
      m_ctrlSlider.PostMessage(WM_LBUTTONDOWN, MAKEWPARAM(MK_LBUTTON, 0), MAKELPARAM(pt.x, pt.y));
      // The timer keeps recording the thumb position. Only Vista has an actual
      // control notification for this!
      SetTimer(UPDATE_TIMERID, 50);
      return 0;
   }
   ...
   {
      CRect rcClient(0, 0, LOWORD(lParam), HIWORD(lParam));
      rcClient.InflateRect(1, 1); // HACK: Get rid of focus border
      m_ctrlSlider.MoveWindow(&rcClient);
      return 0;
   }
   ...
   LRESULT OnTimer(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
   {
      ...
      if( wParam == UPDATE_TIMERID && iPos != m_iLastPos ) {
         m_iLastPos = iPos;
         _Photos.SendChangeNotify(WM_USER_ZOOM_CHANGE, 0, iPos);
      }
      return 0;
   }
   ...
   LRESULT OnSliderKillFocus(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
   {
      KillTimer(UPDATE_TIMERID);
      _Photos.SendChangeNotify(WM_USER_ZOOM_CHANGE, 1, m_ctrlSlider.GetPos());
